...
CombatManeuverReturns PlayerbotDruidAI::DoNextCombatManeuverPVE(Unit* pTarget)
{
	...
	if (m_ai->IsHealer() && m_ai->GetCombatStyle() != PlayerbotAI::COMBAT_RANGED  && !meleeReach)
...
CombatManeuverReturns PlayerbotDruidAI::_DoNextPVECombatManeuverSpellDPS(Unit* pTarget)
{
	...
	if (HURRICANE > 0 && m_ai->In_Reach(pTarget, HURRICANE) && m_ai->GetAttackerCount() >= 5 && CastSpell(HURRICANE, pTarget))
	{
		m_ai->SetIgnoreUpdateTime(10);
		return RETURN_CONTINUE;
	}
...
CombatManeuverReturns PlayerbotDruidAI::HealPlayer(Player* target)
{
	...
	*/
	...
	if (hp < 30 && SWIFTMEND > 0 && m_ai->In_Reach(target, SWIFTMEND) && (target->HasAura(REJUVENATION) || target->HasAura(REGROWTH)) && !m_bot->HasSpellCooldown(SWIFTMEND) && CastSpell(SWIFTMEND, target))
		return RETURN_CONTINUE;
	
	if (hp < 65 && REGROWTH > 0 && m_ai->In_Reach(target, REGROWTH) && !target->HasAura(REGROWTH) && CastSpell(REGROWTH, target))
		return RETURN_CONTINUE;
	else if (hp < 65 && HEALING_TOUCH > 0 && m_ai->In_Reach(target, HEALING_TOUCH) && CastSpell(HEALING_TOUCH, target))
		 return RETURN_CONTINUE;
...
void PlayerbotDruidAI::DoNonCombatActions()
{
	...
	//if (INNERVATE && m_ai->In_Reach(m_bot,INNERVATE) && !m_bot->HasAura(INNERVATE) && m_ai->GetManaPercent() <= 10 && CastSpell(INNERVATE, m_bot))
	//  return;
