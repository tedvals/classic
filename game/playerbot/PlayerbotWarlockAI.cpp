...
CombatManeuverReturns PlayerbotWarlockAI::DoNextCombatManeuverPVE(Unit *pTarget)
{
	...
	if (DRAIN_LIFE &&  !pTarget->HasAura(DRAIN_LIFE) && m_ai->GetHealthPercent() <= 30)
	{
	...
	case WARLOCK_SPEC_AFFLICTION:
		...
		if (DRAIN_LIFE && m_ai->In_Reach(pTarget, DRAIN_LIFE) && m_ai->GetHealthPercent() < 30 && CastSpell(DRAIN_LIFE, pTarget))
		{
			//m_ai->SetIgnoreUpdateTime(5);
			return RETURN_CONTINUE;
		}
	...
	case WARLOCK_SPEC_DEMONOLOGY:	
		//if (CURSE_OF_SHADOW && m_ai->In_Reach(pTarget, CURSE_OF_SHADOW) && !pTarget->HasAura(CURSE_OF_SHADOW) && CastSpell(CURSE_OF_SHADOW, pTarget))
		//	return RETURN_CONTINUE;
		...
		if (IMMOLATE && m_ai->In_Reach(pTarget, IMMOLATE) && !m_ai->HasAuraMY(IMMOLATE, *pTarget, m_botguid) && CastSpell(IMMOLATE, pTarget))
			return RETURN_CONTINUE;
	...
	case WARLOCK_SPEC_DESTRUCTION:
		...
		//if (CURSE_OF_THE_ELEMENTS && m_ai->In_Reach(pTarget, CURSE_OF_THE_ELEMENTS) && !pTarget->HasAura(CURSE_OF_THE_ELEMENTS) && CastSpell(CURSE_OF_THE_ELEMENTS, pTarget))
		//	return RETURN_CONTINUE;
		...
		if (CONFLAGRATE && m_ai->In_Reach(pTarget, CONFLAGRATE) && m_ai->HasAuraMY(IMMOLATE, *pTarget, m_botguid) && !m_bot->HasSpellCooldown(CONFLAGRATE) && CastSpell(CONFLAGRATE, pTarget))
			return RETURN_CONTINUE;
...
bool PlayerbotWarlockAI::CheckCurse(uint32 spec, Unit* pTarget)
{
	...
	if (pCreature && pCreature->IsWorldBoss())
	{
		if (m_bot->GetGroup())
		{
			...
			if ((mages > 1 ) || (spec == WARLOCK_SPEC_DESTRUCTION))
			{
				if (!pTarget->HasAura(CURSE_OF_THE_ELEMENTS))
					CurseToCast = CURSE_OF_THE_ELEMENTS;
				else
					CurseToCast = CURSE_OF_SHADOW;
			}
			else if ((warlocks > 1) || (spec != WARLOCK_SPEC_DESTRUCTION))
			{
				if (!pTarget->HasAura(CURSE_OF_SHADOW))
					CurseToCast = CURSE_OF_SHADOW;
			}
			else
...
void PlayerbotWarlockAI::DoNonCombatActions()
{
	...
	CheckDemon();
	// buff myself DEMON_SKIN, DEMON_ARMOR- Strongest one available is chosen
	if (DEMON_ARMOR)
	{
