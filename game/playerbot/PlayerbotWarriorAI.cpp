...
return RETURN_CONTINUE;
	...
	else if (WAR_STOMP > 0 && !m_bot->HasSpellCooldown(WAR_STOMP) && meleeReach && m_ai->CastSpell(WAR_STOMP, *pTarget))
		return RETURN_CONTINUE;
	...
	case WARRIOR_SPEC_ARMS:
		...
		if (EXECUTE > 0 && (pTarget->GetHealthPercent() < 20 && m_ai->GetRageAmount() < 30 && m_ai->CastSpell(EXECUTE, *pTarget)))
			return RETURN_CONTINUE;
		...
		if (MORTAL_STRIKE > 0 && !m_bot->HasSpellCooldown(MORTAL_STRIKE) && m_ai->CastSpell(MORTAL_STRIKE, *pTarget))
			return RETURN_CONTINUE;		
		// No way to tell if overpower is active (yet)
		...
		if (!newTarget)
			 {
				...
				if (m_ai->GetCombatOrder() & PlayerbotAI::ORDERS_TANK && TAUNT > 0 && !m_bot->HasSpellCooldown(TAUNT) && m_ai->CastSpell(TAUNT, *pTarget))
				 return RETURN_CONTINUE;
					...
					return RETURN_CONTINUE;
				else if (CHALLENGING_SHOUT > 0 && m_ai->GetAttackerCount() >= 2 && !m_bot->HasSpellCooldown(CHALLENGING_SHOUT) && m_ai->CastSpell(CHALLENGING_SHOUT, *pTarget))
					return RETURN_CONTINUE;
